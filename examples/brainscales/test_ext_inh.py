...
connections={}
connections['e2e'] = sim.Projection(exc_cells, exc_cells, full_conn, exc_syn, receptor_type='excitatory')
connections['e2i'] = sim.Projection(exc_cells, inh_cells, full_conn, exc_syn, receptor_type='excitatory')
connections['i2e'] = sim.Projection(inh_cells, exc_cells, full_conn, inh_syn, receptor_type='inhibitory')
connections['i2i'] = sim.Projection(inh_cells, inh_cells, full_conn, inh_syn, receptor_type='inhibitory')
if (options.benchmark == "COBA"):
...
if options.plot_figure:
    ...
    data_stim = ext_stim.get_data().segments[0]
    ...
    ext_weights_exc = nan_to_num(connections['ext2e'].get('weight', format="array"))
    ext_weights_inh = nan_to_num(connections['ext2i'].get('weight', format="array"))
    exc_weights_exc = nan_to_num(connections['e2e'].get('weight', format="array"))
    exc_weights_inh = nan_to_num(connections['e2i'].get('weight', format="array"))
    inh_weights_exc = nan_to_num(connections['i2e'].get('weight', format="array"))
    inh_weights_inh = nan_to_num(connections['i2i'].get('weight', format="array"))
 
    filename_conn = options.plot_figure + "_conn"
    filename_act = options.plot_figure + "_act"
 
    Figure(
        Panel(ext_weights_exc,data_labels=["ext->exc"], line_properties=[{'xticks':True, 'yticks':True, 'cmap':'Greys'}]),
        Panel(ext_weights_inh,data_labels=["ext->inh"], line_properties=[{'xticks':True, 'yticks':True, 'cmap':'Greys'}]),
        Panel(exc_weights_exc,data_labels=["exc->exc"], line_properties=[{'xticks':True, 'yticks':True, 'cmap':'Greys'}]),
        Panel(exc_weights_inh,data_labels=["exc->inh"], line_properties=[{'xticks':True, 'yticks':True, 'cmap':'Greys'}]),
